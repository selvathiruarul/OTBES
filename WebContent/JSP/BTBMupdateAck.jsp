...
<div id='main'>
<p align="right"><a
	href="<%=request.getContextPath()%>/JSP/BTBMupdate.jsp"><b>Update
Another Bill</b></a></p>
<fieldset><legend><%=resourceBundle.getString("billGeneration")%></legend>
<h3><%=resourceBundle
					.getString("acknowledgementForBillUpdation")%></h3>
<br></br>

<%
	BillItemVO billitem = (BillItemVO) request
			.getAttribute("billitems");
	int billid = billitem.getBillID();
	int billitemid = billitem.getBillItemId();
%>

<p><%=resourceBundle.getString("billItemId")%>:<%=billitem.getBillItemId()%></p>
<p><%=resourceBundle.getString("billId")%> :<%=billitem.getBillID()%></p>
<p><%=resourceBundle
					.getString("updationForAboveBillIsSuccessfull")%></p>
</fieldset>
</div>
</div>
</div>
