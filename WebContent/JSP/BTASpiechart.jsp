...
<script type="text/javascript"> 
    ...
    list=(ArrayList<Customerreportvo>)request.getAttribute("list");
    Customerreportvo v=null;
     int count=0;%>
...
function drawChart(){  
 ...
 ['Task', 'Hours per Day'], 
 <% for(count = 0 ; count<(list.size()-1);count++)   
 {
	 v = (Customerreportvo)list.get(count);	
 %>
['<%=v.getState()%>',  <%=v.getCount()%>],
<%System.out.println("lcoation" +v.getState()+"number"+v.getCount());%>
<%}
v = (Customerreportvo)list.get(count);%>
['<%=v.getState()%>',  <%=v.getCount()%>]
 ]);
...
var options = {title: 'Location Wise Percentage of Customers',is3D:true};    
