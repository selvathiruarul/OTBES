...
<div id="main">
<fieldset><legend><%=resourceBundle.getString("StatusUpdateForACustomer")%></legend>
<form name="changestatus" method="post" action="BTASServlet"><input
	type="hidden" name="option" value="statuschange"></input>
<table border="0" cellpadding="10" cellspacing="10" align="center">
	<%
		CustomerVO custstatus = (CustomerVO) request
				.getAttribute("custstatus");
	%>
	<tr>
		<td align="left"><label><%=resourceBundle.getString("customerId")%></label></td>
		<td align="left"><input type="text" name="cid" readonly="readonly"
			value=<%=custstatus.getCustomerID()%>></input></td>
	</tr>
	<tr>
		<%if(custstatus.getStatus().equalsIgnoreCase("Inactive")){ %>
		<td align="left"><label>STATUS :</label></td>
		<td align="left"><select name="statuslist">
			<option value="<%=custstatus.getStatus()%>"><%=custstatus.getStatus()%></option>
...
<input class="button" type="submit" value="<%=resourceBundle.getString("submit")%>"></input></form>
</fieldset>
</div>
</div>
</div>
