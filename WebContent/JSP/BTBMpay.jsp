...
<script type="text/javascript"
	src="<%=request.getContextPath()%>/JS/pay1validation.js"> </script>
...
<div id='main'><p align="right"><a
	href="<%=request.getContextPath()%>/JSP/BTBMViewcid.jsp"><b>Back</b></a></p>


<fieldset><legend><%=resourceBundle.getString("billPayment")%></legend>

<form name="gen" action="BTBMServlet" onsubmit="return validate()"
	method="post"><input type="hidden" name="option" value="payPage"></input>
<%ArrayList<PaymentVO1> v1=(ArrayList<PaymentVO1>)request.getAttribute("Viewdetail"); %>
<%! @SuppressWarnings("unchecked") %>
<label> <%=resourceBundle.getString("billId")%> </label>
 <select name="billId">
                  <%for(PaymentVO1 v:v1){ %>
                  <option value="<%=v.getBillId() %>"><%=v.getBillId() %></option>
                  <%} %>
                  </select><br></br>
                  <input type="submit" value="viewbill" onclick="return validate()" class="button"></input>
