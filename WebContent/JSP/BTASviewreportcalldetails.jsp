...
<div id='main'>
<% 
    ArrayList<CallDetailVO> datalist= (ArrayList<CallDetailVO>)request.getAttribute("report");
if(!datalist.isEmpty()){
	int  conno=(Integer)request.getAttribute("conno");
...
<table border="1" width="1" cellspacing="1" cellpadding="1">
    ...
    for(int i=startindex-1; i<endindex1;i++)
       { 
    	CallDetailVO vo = (CallDetailVO)datalist.get(i);
       %>
		<tr>
			<td><%=i+1%></td>
			...
			<td><%=vo.getCallID()%></td>
			<td><%=vo.getConnectionNo() %></td>
                        <td><%=vo.getDialedNo()%></td>
                        <td><%=vo.getTypeOfCall() %></td>
                        <td><%=vo.getDateOfCall() %></td>
                        <td><%=vo.getDuration() %></td>
                        
                        <td class="td"><a
			href="BTASServlet?id=<%= vo.getCallID() %>&option=updateviewcalldetails">Edit</a></td>
		<td class="td"><a href="BTASServlet?id=<%= vo.getCallID() %>&option=deletecalldetails">Del</a></td>
...
<table width="100%">
	...
	<tr>
		...
		<%if(totalnoofrec>3) {%>
		<%if(startindex != 1) {%>
		<% String strUrl = "BTASServlet?option=generatereportcalldetails&connectionNo="+conno+"&currentpage="+(currentpage-1)+"&noofrecperpage="+noofrecperpage;%>
		...
		<%=i2%>
		<%} else {
                    String strUrl = "BTASServlet?option=generatereportcalldetails&connectionNo="+conno+"&&currentpage="+i2+"&noofrecperpage="+noofrecperpage;
                    %>
		<a href=<%=strUrl%>> <%=i2%></a>
		<%}
                         }%>
